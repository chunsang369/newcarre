...
async function crawl() {
  ...
  for (const maker of makers) {
    ...
    // We'll try up to 3 pages just in case 100 limit still exists
    for (let page = 1; page <= 3; page++) {
      const url = `https://m.hicarzautoplan.com/cars/index/index/?layout=clear&search[where][idxMaker]=${maker.id}&limit=100&page=${page}`;
      ...
      try {
        ...
        console.log(`  Page ${page}: Found ${fragments.length} items, New unique items: ${newFoundThisPage}`);
        if (fragments.length < 10 || newFoundThisPage === 0) break;
      } catch (e) {
