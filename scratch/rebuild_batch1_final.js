...
// Robust extraction from Next.js payload
async function fetchRobustData(trimId) {
    try {
        ...
        // Use exact lookaheads to get full JSON arrays
        try {
        ...
        try {
            const innerRaw = decoded.match(/"trim_inner_color_list":(\[.*?\]),"trim_option/);
            if (innerRaw) innerColors = JSON.parse(innerRaw[1]);
        } catch(e) {}
        
        try {
            const optRaw = decoded.match(/"trim_option_list":(\[.*?\]),"car_brand/);
            if (optRaw) options = JSON.parse(optRaw[1]);
        } catch(e) {}
...
async function runBatch1() {
    console.log('============================================');
    console.log('  배치 1 (1~20대) 완벽 1:1 데이터 매핑 시작');
    console.log('============================================');
    ...
    for (let i = 0; i < batch.length; i++) {
        ...
        if (mainData.trims.length === 0) {
            console.log(`  -> 트림 데이터 없음. 원본 구조 유지`);
            ...
            console.log(`  -> 실제 트림 ID ${mainData.trims.length}개 발견! 1:1 매핑 시작`);
            const newGrades = [];
            if (car.grades) {
    ...
    fs.writeFileSync('scratch/batch1_final.json', JSON.stringify(updatedCars, null, 2));
    console.log('\n✅ 배치 1 (1~20대) 완벽 1:1 매핑 저장 완료: scratch/batch1_final.json');
}
