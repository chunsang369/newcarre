...
async function main() {
  ...
  for (const pt of purchaseTypes) {
    ...
    await sleep(1500); // 탭 전환 및 렌더링을 위해 넉넉히 대기
      ...
      for (const dist of distances) {
        ...
        for (const cond of conditions) {
          ...
          const priceText = await page.evaluate(() => {
            // p.text-right.text-base > span.text-2xl.font-bold.text-c-primary 엘리먼트 추출
            const pElement = document.querySelector('p.text-right.text-base.text-c-title');
            if (pElement) {
              const span = pElement.querySelector('span.text-c-primary');
              if (span) return span.innerText.trim();
            }
            
            // fallback: 하단바 가격
            const bottomBar = document.querySelector('div.fixed.bottom-\\[55px\\]');
            if (bottomBar) {
              const span = bottomBar.querySelector('span.text-c-primary');
              if (span) return span.innerText.trim();
            }
