...
async function crawl() {
  ...
  for (const maker of makers) {
    console.log(`Crawling ${maker.name}...`);
    for (let page = 1; page <= 5; page++) {
      const url = `https://m.hicarzautoplan.com/cars/index/index/?layout=clear&search[where][makerNationCode]=${maker.nation}&search[where][idxMaker]=${maker.id}&page=${page}`;
      ...
      try {
        ...
        console.log(`  Page ${page}: Total items in HTML: ${count}, New items: ${newFound}`);
  ...
  fs.writeFileSync('cars_final_database.json', JSON.stringify(allCars, null, 2), 'utf8');
}
